...
async function getUserCart(userId) {
  // .populate({path:'items.book' , populate:[{path:'author'} , {path:'category'}]})
  const cartDetails = await carts.findOne({user: userId}).populate('items.book');
  if (!cartDetails) {
...
async function addUserCart(userId, addedItems) {
  ...
  const addedBook = await books.findById(addedItems.book);
...
async function updateBookQuantity(userId, bookId, newQuantity) {
  ...
  const editedBook = await books.findById(bookId);
