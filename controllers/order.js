...
async function getUserOrders(userId, pageNumber) {
  ...
  const userOrders = await orders.find({user: userId}).populate('user').populate('items.book').skip(skip).limit(limit).sort({createdAt: -1});
  return {
...
async function getSpecificOrder(user, orderId) {
  const order = await orders.findById(orderId).populate('user').populate('items.book');
  if (!order) {
